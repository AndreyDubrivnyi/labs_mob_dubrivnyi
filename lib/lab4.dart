...
class Lab4 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MyLab4(title: 'Lab 4 Dubrivnyi Andriy'),
    );
  }
}
...
class _MyLab4PageState extends State<MyLab4> {
  ...
  Future<String> readValues() async {
    try {
      ...
      return 'You need to go to lab 3 then do the task';
    }
  }
  @override
  Widget build(BuildContext context) {
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.done) {
              if (snapshot.hasError) {
                return Text('You need to go to lab 3 then do the task');
              } else if (snapshot.hasData) {
                return SingleChildScrollView(
                  child: Text(snapshot.data ?? 'Нет данных'),
                );
              }
            }
            ...
            FutureBuilder<List<FlSpot>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return Center(child: Text('You need to go to lab 3 then do the task'));
                } else if (snapshot.hasData) {
                  return LineChart(
                    LineChartData(
                      lineBarsData: [
